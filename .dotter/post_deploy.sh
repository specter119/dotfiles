...
if [[ "$ENABLED_PACKAGES" == *" claude-code "* ]] && type -P claude >/dev/null 2>&1; then
	if [[ -n "$MORPH_KEY" && -n "$EXA_KEY" ]]; then
		...
		fish -c 'claude mcp add --scope user morph-mcp -e MORPH_API_KEY="$MORPH_KEY" -e ENABLED_TOOLS=edit_file,warpgrep_codebase_search -- bunx @morphllm/morphmcp >/dev/null'
		fish -c 'claude mcp remove --scope user exa >/dev/null 2>&1; or true'
		fish -c 'claude mcp add --scope user exa -e EXA_API_KEY="$EXA_KEY" -- bunx exa-mcp-server "tools=web_search_exa,get_code_context_exa,crawling_exa" >/dev/null'
...
if [[ "$ENABLED_PACKAGES" == *" codex "* ]] && type -P codex >/dev/null 2>&1; then
	if [[ -n "$MORPH_KEY" && -n "$EXA_KEY" && -n "$CONTEXT7_KEY" ]]; then
		...
		fish -c 'codex mcp add morph-mcp --env MORPH_API_KEY="$MORPH_KEY" --env ENABLED_TOOLS=edit_file,warpgrep_codebase_search -- bunx @morphllm/morphmcp >/dev/null'

		fish -c 'codex mcp remove exa >/dev/null 2>&1; or true'
		fish -c 'codex mcp add exa --env EXA_API_KEY="$EXA_KEY" -- bunx exa-mcp-server "tools=web_search_exa,get_code_context_exa,crawling_exa" >/dev/null'

		fish -c 'codex mcp remove context7 >/dev/null 2>&1; or true'
		fish -c 'codex mcp add context7 -- bunx @upstash/context7-mcp --api-key "$CONTEXT7_KEY" >/dev/null'
