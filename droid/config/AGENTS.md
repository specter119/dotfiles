...
3. **分解委派** - 非 trivial 操作优先委派 subagent，主 agent 只做轻量确认、汇总和裁决
...
3. 代码搜索、现状梳理、找入口、多文件对比 → 委派 `explorer`（可并行多个）
4. 外部文档、某库/API 用法 → 委派 `librarian`
5. 明确实现任务，且受益于隔离执行、独立验证或并行推进 → 委派 `worker`
6. 多个互不依赖的子任务 → 并行委派多个 subagent（文件范围不重叠）
...
2. 是否受益于隔离执行或并行？如果是，优先委派 subagent
3. 是否在解释已知的基础知识？
4. 是否可以直接修复低级错误？
